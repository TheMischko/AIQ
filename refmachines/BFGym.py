...
class BFGym(ReferenceMachine,Env):
    ...
    ## Base AIQ method returns tuple of reward and observations.
    ...
    def reset( self, seed="" , program="", options = None ):
        self.program = program
        self.init_machine()
        # print( 0.0, [self.mid_symbol]*self.obs_cells)
    ...
    def step(self, action):
        reward, observations, steps = self.act( action)
        return observations, reward, True if action >= self.num_actions else False, ""
